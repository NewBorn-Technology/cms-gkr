...
export default function CreateDevotionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await devotionService.createDevotion(formData);
      if (response.success) {
        toast.success('Devotion created successfully');
        router.push('/devotions');
      }
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to create devotion');
    } finally {
      setLoading(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Create New Devotion</h1>
            <p className="mt-2 text-sm text-gray-600">
              Fill in the details below to create a new daily devotion.
            </p>
          </div>
        </div>
